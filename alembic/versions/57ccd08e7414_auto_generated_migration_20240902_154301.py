...
def upgrade() -> None:
    ...
    op.drop_index('ix_task_log_tlog_id', table_name='task_log')
    op.drop_index('ix_task_log_tlog_name', table_name='task_log')
    op.drop_index('ix_task_log_tlog_status', table_name='task_log')
...
def downgrade() -> None:
    ...
    sa.Column('tlog_id', postgresql.UUID(), autoincrement=False, nullable=False),
    sa.Column('tlog_name', sa.VARCHAR(), autoincrement=False, nullable=True),
    sa.Column('tlog_status', sa.VARCHAR(), autoincrement=False, nullable=True),
    sa.Column('tlog_type', sa.VARCHAR(), autoincrement=False, nullable=True),
    sa.Column('tlog_details', postgresql.JSONB(astext_type=sa.Text()), autoincrement=False, nullable=True),
    sa.Column('tlog_start_time', postgresql.TIMESTAMP(timezone=True), autoincrement=False, nullable=True),
    sa.Column('tlog_end_time', postgresql.TIMESTAMP(timezone=True), autoincrement=False, nullable=True),
    sa.Column('tlog_success', sa.BOOLEAN(), autoincrement=False, nullable=True),
    sa.Column('tlog_error_message', sa.TEXT(), autoincrement=False, nullable=True),
    sa.Column('tlog_error_trace', sa.TEXT(), autoincrement=False, nullable=True),
    sa.Column('tlog_inserted_at', postgresql.TIMESTAMP(timezone=True), server_default=sa.text('now()'), autoincrement=False, nullable=True),
    sa.PrimaryKeyConstraint('tlog_id', name='task_log_pkey')
    )
    op.create_index('ix_task_log_tlog_status', 'task_log', ['tlog_status'], unique=False)
    op.create_index('ix_task_log_tlog_name', 'task_log', ['tlog_name'], unique=False)
    op.create_index('ix_task_log_tlog_id', 'task_log', ['tlog_id'], unique=False)
    # ### end Alembic commands ###
