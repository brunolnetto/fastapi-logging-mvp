...
async def request_to_log(
    request_start_time: float, request: Request, response: Response
) -> Dict[str, Any]:
    ...
    return {
        ...
        "inserted_at": time.strftime(
            "%Y-%m-%d %H:%M:%S", time.localtime(request_start_time)
        ),
    }
